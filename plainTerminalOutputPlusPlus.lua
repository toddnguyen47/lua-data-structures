---Base on busted's plainTerminal output handler
---Ref: https://github.com/Olivine-Labs/busted/blob/master/busted/outputHandlers/plainTerminal.lua
local s = require 'say'
local pretty = require 'pl.pretty'
local io = io

---@param options string A string containing options passed to this output handler.
-- To pass arguments to this function, use `-Xoutput "[comma,separated,lists]"`, where the string
-- will be split based on the commas and converted into a list (table in Lua)
-- Here are the options as well as their indices:
-- [1] -> successDot
-- [2] -> failureDot
-- [3] -> errorDot
-- [4] -> pendingDot
return function(options)
  ...
  local pendingDescription = function(pending)
    ...
    local string = s('output.pending') .. ' -> ' .. pending.trace.short_src .. ' @ ' ..
